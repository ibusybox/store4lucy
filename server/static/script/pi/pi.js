...
function handleGetPIByCountIndexHTML(){
    ...
    for ( var i = 0 ; i < countList.length; i++ ){
        $.get( URL_QUERY_PI_BY_COUNT_INDEX_JSON + '/?current=' + i, function( pi ){
            ...
            var html = new EJS({url: '/script/pi/pi_summary.ejs'}).render(renderData);
            if ( count % 4 === 0 ){
...
function handleGetPIDetailByPINOHTML(){
    ...
    $.get( URL_QUERY_PI_DETAIL_BY_NO_WITH_PRODUCTCONTENT_JSON + '/?pi_no=' + piNO, function( PI ){
        ...
        for ( var i = 0; i < PI.product_purchase_list.length; i++ ){
            ...
            purchaseInfoHTML = new EJS({url: '/script/pi/pi_purchase_info.ejs'}).render(renderData);
            if ( count % 4 === 0 ){
        ...
        renderData = {pi: PI};
        var PISummaryHTML = new EJS({url: '/script/pi/pi_purchase_summary.ejs'}).render(renderData);
        ...
        //setup button events
        //1. export PI as quatation to PDF
        $('#export_pi_quatation').bind('click', function( event ){
            event.stopPropagation();
            window.location.href = URL_EXPORT_PI_AS_QUATATION + '/?pi_no=' + $('#pi_purchase_summary input[name=pi_no]').val();
        });
    } );
}
